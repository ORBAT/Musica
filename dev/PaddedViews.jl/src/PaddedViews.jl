...
using OffsetArrays, Debugger, Infiltrator
...
"""
Any type that can be treated like an array.
"""
const ArrayLike{T,N} = Union{<:AbstractArray{T,N}, <:NTuple{N, T}}
...
struct PaddedView{T,N,I,A} <: AbstractArray{T,N}
    ...
    function PaddedView{T,N,I,A}(fillvalue,
        ...
        dims(data) == N || throw(DimensionMismatch("data and indices should have the same dimension, instead they're $(dims(data)) and $N."))
        new{T,N,I,A}(fillvalue, data, indices)
    end
end

function PaddedView(fillvalue::FT, data::ArrayLike{T}, args...) where {FT, T}
...
function PaddedView{FT}(fillvalue,
    ...
    inds = map(OneTo, sz)
    PaddedView{FT,dims(data),typeof(inds),typeof(data)}(convert(FT, fillvalue), data, inds)
end
...
@inline dims(@nospecialize(_::Tuple)) = 1
@inline dims(v) = ndims(v)

@inline Base.axes(A::PaddedView) = A.indices
@inline Base.axes(A::PaddedView, d::Integer) = d <= dims(A) ? A.indices[d] : default_axes(A.indices)
@inline default_axes(::NTuple{N,I}) where {N,I<:AbstractUnitRange} = convert(I, OneTo(1))
@inline default_axes(::Tuple{}) = OneTo(1)
@inline default_axes(::Any) = OneTo(1)

Base.size(A::PaddedView) = map(length, axes(A))

Base.parent(A::PaddedView) = A.data

_checkbounds(::Type{Bool}, ::NTuple{N}, i) where N = 1 ≤ i ≤ N
_checkbounds(::Type{Bool}, ::NTuple{N}, I...) where N = (@bp;Base.checkbounds_indices(Bool, (OneTo(N),), I))
_checkbounds(::Type{Bool}, v, I...) = Base.checkbounds(Bool, v, I...)
_checkbounds(v, I...) = _checkbounds(Bool, v, I...) || Base.throw_boundserror(A,I)
Base.IndexStyle(::Type{<:PaddedView{T,N,I,A}}) where {T,N,I,A} = Base.IndexStyle(A)
# Base.IndexStyle(::PaddedView{T,N,I,A}) where {T,N,I,A} = IndexLinear()

Base.@propagate_inbounds function Base.getindex(A::PaddedView{T,N}, i::Vararg{Int,N}) where {T,N}
    # @info "PaddedView getindex vararg" A.data i
    # @bp
    # @infiltrate
    @boundscheck _checkbounds(A, i...)
    if _checkbounds(Bool, A.data, i...)
        return convert(T, A.data[i...])
    end
    return A.fillvalue
end

# Base.@propagate_inbounds function Base.getindex(A::PaddedView{T,N,I,A}, i...) where {T,N,I,A <: NTuple}
#     # @info "PaddedView getindex" A.data i
#     # @bp
#     @boundscheck _checkbounds(A, i...)
#     if _checkbounds(Bool, A.data, i...)
#         return convert(T, A.data[i...])
...
```
"""
function paddedviews(fillvalue,  As::AbstractArray...; dims=1:dims(first(As)))
    inds = outerinds(As...)
...
function _extended_axes(A, inds, dims::Int)
    map((r1, r2, d)->d ? r2 : r1, axes(A), inds, ntuple(i->i==dims, dims(A)))
end
function _extended_axes(A, inds, dims::Tuple)
    map((r1, r2, d)->d ? r2 : r1, axes(A), inds, ntuple(i->i in collect(dims), dims(A)))
end
...
julia> a2f
 ...
 ```
"""
function sym_paddedviews(fillvalue, As::AbstractArray...; dims=1:dims(first(As)))
    inds = outerinds(As...)
    map(As) do A
...
function _sym_pad_inds(A, inds, dims=1:dims(A))
