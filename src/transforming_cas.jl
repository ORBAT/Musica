struct Stateful{Len,C<:Abstract,State} <: Abstract
  ...
  function Stateful(ca::C, initial_state::Row{NStates,Len}) where {NStates,Len,C<:Abstract}
    # TODO: make sure ca accepts initial_state
...
@inline apply_ca!(sca::Stateful{L}, inp::Row{N,W}) where {L,N,W} =
  apply_ca!(sca.ca, sca.state ⊻ inp)
...
@testitem "Stateful" begin
  let ca = CA.Elementary(110), init_state = CA.num_to_row(5, Val(16)),
    test_state = CA.num_to_row(1, Val(16))
...
HasParent(::Type{Repeated{CA}}) where {CA} = ParentType(CA)
