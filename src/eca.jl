struct Elementary{NStates,Radius,RuleLen} <: Abstract
  ...
  """
  Base.@propagate_inbounds function Elementary(rule)
    @boundscheck @assert 0 ≤ rule < 256
    new{2,1,0}(rule, SVector{0}())
  end

  Base.@propagate_inbounds function Elementary{NStates,Radius}(rule) where {NStates,Radius}
...
Elementary{2,1}(rule) = Elementary(rule)

function Elementary{NStates}(rule) where {NStates}
  Elementary{NStates,1}(rule)
end
...
@inline _toroidal_iter(::Val{R}, r::Row{N,L}) where {R,N,L} =
  SOneTo{L + 2R}() |>
  Map(
    #= (idx -> @inbounds r[idx]) ∘
    @<(_to_toroidal_idx(R, L)) =#
    idx -> @inbounds r[Musica._to_toroidal_idx(idx, R, L)]
  ) |>
  Consecutive(Val(neighborhood_size(R)), Val(1))

@inline function apply_ca!(ca::Elementary{NS,Rad}, row::Row{NS}) where {NS,Rad}
  _collect_into!(row, _eduction(ca, row))
end

@inline function _eduction(ca::Elementary{NS,Rad}, row::Row{NS}) where {NS,Rad}
  _toroidal_iter(Val(Rad), row) |> Map(@>(apply_rule(ca)))
end

"collects `eduction` into `dest`. Mutates `dest`"
@inline _collect_into!(dest, eduction) =
  foldxl(eduction |> Enumerate(); init=dest) do acc, (idx, a)
    @inline
    # folds eduction into collection
    @inbounds acc[idx] = a
    acc
  end


@propagate_inbounds function apply_rule(dca::Elementary{NS}, x) where {NS}
  # +1 because array indexing starts at 1
  dca.rule_lookup[undigits(x, Val(NS))+1]
end
...
Base.@constprop :aggressive @inline function rule_to_rule_lookup(rule, nstates=2, radius=1)
  # FIXME: type unstable jos annetaan nstates ja radius
...
# @inline function parser(::Type{<:Elementary}; kw...)
#   Parser() do bits
#     (bits |> Drop(8) |> collect, Elementary(undigits(bits |> Take(8) |> collect)))
#   end
# end

# @testitem "parser" begin
#   @test digits(110; base=2) |> Musica.parser(Elementary) == (Bool[], Elementary(110))
# end

# @inline parser_bits_required(::Type{<:Elementary}; kw...) = 8
